...
chrome.storage.sync.get("defaultSpeed", (data) => {
    if (data.defaultSpeed) {
        // Make sure there's a video element before applying speed
        const video = document.querySelector("video");
        if (video) {
            applySpeed(data.defaultSpeed); // Apply default speed if it's set
        } else {
            // If no video yet, set up an observer to apply speed when video appears
            const videoObserver = new MutationObserver((mutations, obs) => {
                const video = document.querySelector("video");
                if (video) {
                    applySpeed(data.defaultSpeed);
                    obs.disconnect(); // Stop observing once speed is applied
                }
            });
...
function addSpeedControlButtons() {
    ...
    if (
        activeVideo &&
        titleContainer &&
        !document.getElementById("speed-control-btn")
    ) {
